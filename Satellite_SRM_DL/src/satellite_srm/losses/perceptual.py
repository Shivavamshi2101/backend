...
class PerceptualVGGLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # Reorder to [R, G, B]
        pred_rgb = torch.from_numpy(pred.detach().cpu().numpy()[:, [2, 1, 0], :, :]) if hasattr(pred, "detach") else pred[:, [2, 1, 0], :, :]
        targ_rgb = torch.from_numpy(target.detach().cpu().numpy()[:, [2, 1, 0], :, :]) if hasattr(target, "detach") else target[:, [2, 1, 0], :, :]
